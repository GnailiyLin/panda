...
func TestBuiltinFunctions(t *testing.T) {
	tests := []vmTestCase{
		...
		{`len("hello world")`, 11},
		{
			`len(1)`,
			&object.Error{
				Message: "argument to `len` not supported, must to be Array, got=Integer",
			},
		},
		{`len("one", "two")`,
			&object.Error{
...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case map[object.HashKey]int64:
		...
		if len(hash.Pairs) != len(expected) {
			t.Errorf("hash has wrong number of Paris. want=%d, got=%d", len(expected), len(hash.Pairs))
			return
		}
